...
(function($) {
	function _4b(_4c) {
		var opts = $.data(_4c, "linkbutton").options;
		$(_4c).empty();
		$(_4c).addClass("l-btn");
		//add by qsd 20120405_1 begin

		//add by qsd 2012-5-25_4 begin
		var css_l_btn = "l-btn-"+opts.type;
		var css_l_btn_left="l-btn-left-"+opts.type;
		/*
		//add by qsd 2012-5-25_4 end
		
		var css_l_btn = "l-btn-ellipse";
		var css_l_btn_left="l-btn-left-ellipse";
		if(opts.type=="rectangle"){
			css_l_btn ="l-btn-rectangle";
			css_l_btn_left ="l-btn-left-rectangle";
		}else if(opts.type=="rect_crystal"){
			css_l_btn ="l-btn-rect-crystal";
			css_l_btn_left ="l-btn-left-rect-crystal";
		}
			//add by qsd 20120514_1 begin    
			else if(opts.type=="short_rect"){
				css_l_btn ="l-btn-short-rect";
				css_l_btn_left ="l-btn-left-short-rect";
			}else if(opts.type=="short_eclipse"){
				css_l_btn ="l-btn-short-eclipse";
				css_l_btn_left ="l-btn-left-short-eclipse";
			}else if(opts.type=="backt"){
				css_l_btn ="l-btn-backt";
				css_l_btn_left ="l-btn-left-backt";
			}
			//add by qsd 20120514_1 end
		
		//add by qsd 2012-5-25_4 begin
		*/
		//add by qsd 2012-5-25_4 end
		
		$(_4c).addClass(css_l_btn);
		//add by qsd 20120405_1 end
		...
		if (opts.text) {
			//add by qsd 20120405_1 begin
			/* old
			$(_4c).html(opts.text).wrapInner(
					"<span class=\"l-btn-left\">"
							+ "<span class=\"l-btn-text\">" + "</span>"
							+ "</span>");
			*/
			$(_4c).html(opts.text).wrapInner(
					"<span class=\"l-btn-left "+css_l_btn_left+"\">"
							+ "<span class=\"l-btn-text\">" + "</span>"
							+ "</span>");
			...
			if (opts.iconCls) {
				$(_4c).find(".l-btn-text").addClass(opts.iconCls).css(
						"padding-left", "20px");
			}
		} else {
			//add by qsd 20120405_1 begin
			...
			*/
			...
			//add by qsd 20120405_1 end
			if (opts.iconCls) {
				$(_4c).find(".l-btn-empty").addClass(opts.iconCls);
			}
		}
		_4e(_4c, opts.disabled);
	}
	;
	...
	$.fn.linkbutton.parseOptions = function(_56) {
		...
		return {
			...
			type : (t.attr("type") ? t.attr("type") : "ellipse" ),
			text : $.trim(t.html()),
	...
	$.fn.linkbutton.defaults = {
		...
		disabled : false,
		plain : false,
		type : "ellipse", // first:  plain=false  ::  "ellipse" "rectangle" "rect_crystal" //add by qsd 20120405_1
		//add by qsd 20120514_1  type::  short_rect  short_eclipse  backt
		//add by qsd 2012-5-25_4  全部取消下划线  改为大写开头  rectCrystal shortRect shortEclipse
		text : "",
		iconCls : null
	};
})(jQuery);
...
(function($) {
	...
	;
	//add by qsd 20111125_2begin
	///is array
	function ifArr(value){
		return value && 
			typeof value === 'object' && 
	...
	function _cf(_d0, ul, _d1, _d2) {
		...
		//add by qsd 2012-5-8_2 begin
		if(_d3.useAjaxjson){
			
			var queryPara = {};
			var tmpq = _d3.useAjaxjson_queryPara;
			for(var p in tmpq){
				queryPara[p] = tmpq[p];
			}
			if(!_d1.id) _d1.id = 0;
			eval('queryPara.'+_d3.useAjaxjson_queryPara_superIdAttrName+'=_d1.id;');
			
			_log("min1");
			$.ajaxjson.post({url : _d3.url,
		        queryPara : queryPara,   
				successFunc: function(data){
					var _d7 = [];
					for(var i=0; data && data!=null && i<data.length; i++){
						var d = data[i];
						eval("var tmp = {id:d."+_d3.useAjaxjson_data_id_AttrName+", text:d."+_d3.useAjaxjson_data_text_AttrName+" };");
						//eval("var checked = ''+d."+_d3.useAjaxjson_data_checked_AttrName||'_aa'+"=='true' ? true : false ;");
						eval("var state = ''+d."+_d3.useAjaxjson_data_isNodeLeafAttrName+"=='false' ?'closed':'' ;");
						_d7.push({id: tmp.id, text: tmp.text,  state: state, attributes: d});
					}
					var _d7_all = [];
					if(_d1.id==0 && _d3.root && _d3.root!=null ){
						_d3.root["children"]=_d7;
						_d7_all.push(_d3.root);
					}else{
						_d7_all = _d7;
					}
					/*
					for(var i=0; _d3.data && _d3.data!=null && i<_d3.data.length;i++){
						var td = _d3.data[i];
						if(td.id == _d1.id){
							_d3.data[i]["children"] = _d7;
							break;
						}
						if(checkChildren(td["children"])) break;
					}
					function checkChildren(nodes){
						for(var i=0; nodes && nodes!=null && i<nodes.length;i++){
							var td = nodes[i];
							if(td.id == _d1.id){
								nodes[i]["children"] = _d7;
								return true;
							}
							if(checkChildren(td["children"])) return true;
						}
						return false;
					}
					
					if(!_d3.data || _d3.data==null){
						_d3.data = _d7;
					}*/
					...
					_d3.useAjaxjson_loadOverFunc(data);
				},
				error : function() {
					alert();
					_d6.removeClass("tree-loading");
	...
	$.fn.tree.defaults = {
		...
		root: null,
		useAjaxjson: false,
		useAjaxjson_data_id_AttrName: '_ID',// 必填返回数据组装
		useAjaxjson_data_text_AttrName: 'modelObj.name',// 必填返回数据组装
		useAjaxjson_queryPara : {modelObj:{}},
		useAjaxjson_queryPara_superIdAttrName: 'modelObj.superId',//必填
		useAjaxjson_data_isNodeLeafAttrName: 'isLeaf_tree', //必填 true of false
		//useAjaxjson_data_checked_AttrName: '',// true or false
...
(function($) {
	...
	function _2b3(_2b4) {
		...
		/*----//add by qsd zb 2012-5-25------------------start-----------------------*/
		//setTimeout(function(){
			var center = _2b5.center.panel("panel");		
			var opts = $.data(_2b4, "layout").options;			
			var bg = opts.bg;	// 背景图/色
			var reg = /layout-bg/;
			var bgCover = opts.bgCover;	// layout的背景  是否覆盖  layout中其他layout的背景，默认为true
			if(bg!="default"){
			
				if (reg.test(bg)) {
					$(_2b4).addClass(bg);
				} else {
					$(_2b4).css("background", bg);
				}
				
				if (_2b5.east.length) {
					_2b5.east.css("background", "transparent");
				}
				if (_2b5.west.length) {
					_2b5.west.css("background", "transparent");
				}
				if (_2b5.north.length) {
					_2b5.north.css("background", "transparent");
				}
				if (_2b5.south.length) {
					_2b5.south.css("background", "transparent");
				}
				_2b5.center.css("background", "transparent");
				
				if (bgCover == "true") {				
					center.parent().find("div").css("background", "transparent");
				}
			}
		//}, 1);
		/*-----//add by qsd zb 2012-5-25------------------end-----------------------*/
	}
	;
	function _29e(pp) {
		if (!pp) {
		...
		return this.each(function() {
			...
			if (!_2b8) {
				var opts = $.extend( {}, {
					...
					//add by qsd zb 2012-5-25 begin
					,
					// add attribute(bg、bg_override)
					bg : ($(this).attr("bg") ? $(this).attr("bg") : "default"),
					bgCover : ($(this).attr("bgCover") ? $(this).attr("bgCover") : "true")
					//add by qsd zb 2012-5-25 end
				});
				$.data(this, "layout", {
					options : opts,
...
(function($) {
	...
	function _320(_321) {
		...
		*/
		var opts = $.data(_321, "validatebox").options;
		var direction = opts.direction==""||opts.direction=="right"?"":"-"+opts.direction;
		if (!tip) {
			tip = $(
					"<div class=\"validatebox-tip\">"
							+ "<span class=\"validatebox-tip-content"+direction+"\">"
							+ "</span>"
							+ "<span class=\"validatebox-tip-pointer"+direction+"\">"
							+ "</span>" + "</div>").appendTo("body");
			$.data(_321, "validatebox").tip = tip;
		}
		tip.find(".validatebox-tip-content"+direction).html(msg);
		//add by qsd 20120401_1 end
		
		//add by qsd 20120401_1 lzz 将tip移动到下方，同时需要修改样式 bengin
		/* old
			tip.css( {
				display : "block",
		...
		if(opts.direction==""||opts.direction=="right"){
			...
			tip.css( {
				...
				top : box.offset().top + box.outerHeight() - 1
			});
		...
		//add by qsd 20120401_1 lzz 将tip移动到下方，同时需要修改样式 end
	}
	;
	function _31f(_322) {
	...
	$.fn.validatebox.parseOptions = function(_32e) {
		...
		return {
			...
			direction : (t.attr("direction") || undefined), // add by qsd 20120401_1
	...
	$.fn.validatebox.defaults = {
		...
		invalidMessage : null,
		direction: "down",//""="right"   "down"  "right" // add by qsd 20120401_1
		rules : {
			email : {
...
(function($) {
		...
		if (opts.pagination) {
			...
			_3f3.pagination( {
				...
				onSelectPage : function(_3f4, _3f5) {
					//add by qsd 2012-7-11 start
					opts.onBeforeSelectPage.call(_3ea, _3f4, _3f5);
					//add by qsd 2012-7-11 end
	...
	function _3f9(_3fa) {
		...
		body.find("tr[datagrid-row-index]").unbind(".datagrid")
		.bind(
				"mouseenter.datagrid",
				function() {
		...
		if (!opts.remoteSort) {
			...
			if (opt) {
				...
				data.rows.sort(function(r1, r2) {
					// modify by huangjian 20120806 begin
					/*
					return _440(r1[opts.sortName], r2[opts.sortName])
							* (opts.sortOrder == "asc" ? 1 : -1);
							* */
					var a = eval("r1."+opts.sortName);
					var b = eval("r2."+opts.sortName);
					return _440(a, b)
							* (opts.sortOrder == "asc" ? 1 : -1);
					// modify by huangjian 20120806 end
				});
			}
		}
		var view = wrap.children("div.datagrid-view");
	...
	function _4ae(_4af, _4b0) {
		...
		function _4b3() {
			...
			//add by qsd 2012-5-8 begin
			if(opts.useAjaxjson){
				
				var queryPara = _4b2;
				var pageSize = queryPara.rows;
				var currentPage = queryPara.page;
				var page = opts.useAjaxjson_page_queryPara;
				for(var p in page){
					queryPara[p] = page[p];
				}
				eval('queryPara.'+opts.useAjaxjson_page_pageSize_attrName+'=pageSize;');
				eval('queryPara.'+opts.useAjaxjson_page_currentPage_attrName+'=currentPage;');

				_log("min2");
				$.ajaxjson.post({url : opts.url,
			        queryPara : queryPara,   
					successFunc: function(data){
						setTimeout(function() {
						...
						eval("var dataobj = {total:data."+opts.useAjaxjson_total_attrName+",rows:data."+opts.useAjaxjson_rows_attrName+"};");
						_log("dataobj:" + dataobj);
						_43d(_4af, dataobj);
						setTimeout(function() {
							_4a2(_4af);
						}, 0);
						
						opts.useAjaxjson_loadOverFunc(data);
					},
					errorFunc: function(xmlHttpRequest, textStatus, errorThrown, msgObj){
						setTimeout(function() {
							$(_4af).datagrid("loaded");
						}, 0);
						if (opts.onLoadError) {
			...
			$.ajax( {
				...
				data : _4b2,
				dataType : "json",
				//add by qsd 2012-5-7 begin
				contentType : opts.contentType,
				//add by qsd 2012-5-7 end
				success : function(ret) {
				//add by qsd 20111202-2 beign
				var data = [];
				if(ifArr(ret)){
					data = ret;
				}else{
	...
	$.fn.datagrid.defaults = $.extend( {}, $.fn.panel.defaults, {
		...
		//add by qsd 2012-5-8 begin
		useAjaxjson: false,
		useAjaxjson_total_attrName: 'page.totalItems',// 必填返回数据组装
		useAjaxjson_rows_attrName: 'content',// 必填返回数据组装
		useAjaxjson_page_queryPara: { page:{pageSize:10, currentPage:1 } },
		useAjaxjson_page_pageSize_attrName:'page.pageSize',
		useAjaxjson_page_currentPage_attrName:'page.currentPage',
		useAjaxjson_loadOverFunc: function(data){
		},
		//add by qsd 2012-5-8 end
...
(function($) {
	...
	function _5a7(_5d5, _5d6, _5d7, _5d8, _5d9) {
		...
		_log("ajax---------type:" + opts.method + ",url:" + opts.url + ",data:" + _5da);
		$.ajax( {
			type : opts.method,
			url : opts.url,
			data : _5da,
...
(function($) {
	function _658(_659, _65a) {
		...
		var comboObj = $.data(_659, "combo").combo;
		var _65c = $.data(_659, "combo").panel;
		if (_65a) {
			opts.width = _65a;
		}
		comboObj.appendTo("body");
		if (isNaN(opts.width)) {
			opts.width = comboObj.find("input.combo-text").outerWidth();
		}
		var _65d = 0;
		if (opts.hasDownArrow) {
			_65d = comboObj.find(".combo-arrow").outerWidth();
		}
		var _65a = opts.width - _65d;
		if ($.boxModel == true) {
			_65a -= comboObj.outerWidth() - comboObj.width();
		}
		// add by qsd 20120401_2 lzz begin
		/*old
		comboObj.find("input.combo-text").width(_65a);
		*/
		if (opts.widthAuto) {
			comboObj.find("input.combo-text").width(
					$(_659).parent().outerWidth()
							- comboObj.find(".combo-arrow").outerWidth() - 5);
			var pWidth = $(_659).parent().outerWidth();
			if (opts.widthAutoAlways){
				setInterval(function() {
					if (pWidth != $(_659).parent().outerWidth()) {
						comboObj.find("input.combo-text").width(
								$(_659).parent().outerWidth()
										- comboObj.find(".combo-arrow").outerWidth()
										- 5);
						pWidth = $(_659).parent().outerWidth();
					}
				}, 100);
			}
		} else {
		...
		// add by qsd 20120401_2   end
		_65c.panel("resize", {
			width : (opts.panelWidth ? opts.panelWidth : comboObj.outerWidth()),
			height : opts.panelHeight
		});
		comboObj.insertAfter(_659);
	}
	;
	function _65e(_65f) {
	...
	function _667(_668) {
		...
		if (!opts.disabled) {
			...
			// add by qsd 2012-5-4 begin
			if (!opts.editable) {
				_66c.bind("click.combo", function() {
					if (_66b.is(":visible")) {
						_674(_668);
					} else {
						$("div.combo-panel").panel("close");
	...
	function _66e(_66f) {
		...
		//add by qsd 20111202 begin
		var tmp_id = null;
		try{
			tmp_id = _671.panel("panel").find('iframe')[0].id;
		}catch(e){}
		if(tmp_id!=null){
			try{
				document.getElementById(tmp_id).contentWindow.__onfocusMe();
			}catch(e){
				try{
				frames[tmp_id].__onfocusMe();
				}catch(e){}
			}
		}
		//add by qsd 20111202 end
	...
	$.fn.combo = function(_699, _69a) {
		...
		return this.each(function() {
			...
			if (_69b) {
				...
				$(this).removeAttr("disabled");
				//add by qsd 20111128begin
				if(_699.iframeUrl && _699.iframeUrl!=null && _699.iframeUrl!=""){
					var this_id = this.id;
					var div_id= this_id+'_div_iframe'
					$('body').append('<div id="'+div_id+'" ><iframe id="'+div_id+'_iframe" src="'+_699.iframeUrl+'" width="100%" height="100%" frameborder="0"></iframe></div>');
					//setTimeout(function(){
						$('#'+div_id).appendTo($('#'+this_id).combo('panel'));
					//},1);
				}
				//add by qsd 20111128end
			}
		...
		return $
				.extend(
						...
						$.fn.validatebox.parseOptions(_6a0),
						{
							...
							widthAuto : (t.attr("widthAuto") ? true : undefined),//add by qsd 20120401_2 
							widthAutoAlways : (t.attr("widthAutoAlways") ? true : undefined),//add by qsd 20120401_2 
	...
	$.fn.combo.defaults = $.extend( {}, $.fn.validatebox.defaults, {
		...
		widthAuto: undefined,//add by qsd 20120401_2 
		widthAutoAlways: undefined,//add by qsd 20120401_2 

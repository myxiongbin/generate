...
作者：TNT2 (SZW on cnblogs  - szw.cnblogs.com)  QQ：63408537/498977166（加位好友请说明来意）   Email/MSN:szw2003@163.com    
...
function ShowPageBar(containerId, url, attr) {
    ...
    var showPageNumber = (attr["showPageNumber"] == null || attr["showPageNumber"] < 0) ? 3 : attr["showPageNumber"]; //attr["showPageNumber"];//显示页码数量
    ...
    if (showPageNumber > 0) {//显示多个页码
        ...
        $(GetPageLink(currentPageIndex, currentPageIndex, currentPageIndex, null, null, barMark)).appendTo($("#" + barID));
    }
...
jQuery.fn.extend({
    showPageBar: function(url, attr) { ShowPageBar($(this).attr("id"), url, attr); }
});
(function($) {
	...
	;
	function openWinp(_c, _d, _e, closeable) {
		...
		_f.winp( {
			...
			width : "" + closeable == "undefined" ? 300 : 250,
			height : "auto",
			modal : true,
			collapsible : false,
			minimizable : false,
		...
		_f.winp("winp").addClass("messager-window");
		return _f;
	}
	;
	$.winmess = {
		...
		alert : function(_13, msg, _14, fn) {
			...
			var _16 = {};
			_16[$.winmess.defaults.ok] = function() {
				win.dialog( {
					closed : true
				});
				if (fn) {
		...
		confirm : function(_17, msg, fn) {
			...
			var _19 = {};
			_19[$.winmess.defaults.ok] = function() {
				win.winp("close");
				if (fn) {
					fn(true);
					return false;
			...
			_19[$.winmess.defaults.cancel] = function() {
				win.winp("close");
				if (fn) {
					fn(false);
					return false;
		...
		prompt : function(_1a, msg, fn) {
			...
			var _1c = {};
			_1c[$.winmess.defaults.ok] = function() {
				win.winp("close");
				if (fn) {
			...
			_1c[$.winmess.defaults.cancel] = function() {
				win.winp("close");
				if (fn) {
					fn();
					return false;
		...
		progress : function(_1d) {
			...
			var _21 = "<div class='progress'>" + _1e.msg + "</div>"
			var win = openWinp(_1e.title, _21, null, false);
			win.css("background", "#efefef");
		}
	};
	$.winmess.defaults = {
		ok : "确定",
